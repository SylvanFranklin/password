...
pub fn check_if_json_file_exists() -> bool {
    ...
    let path_str = format!("{}/.config/PasswordManager/passwords.json", &home_dir);
    let path = Path::new(&path_str);
    let hash_path_str = format!("{}/.config/PasswordManager/hash.txt", &home_dir);
...
pub fn create_if_not_exists(new_password: &str) {
    if !check_if_json_file_exists() {
        ...
        let home_dir = env::var("HOME").expect("Failed to get home directory");
        let dir_path = format!("{}/.config/PasswordManager", home_dir);
        ...
        let mut hash_file: File =
            File::create(&format!("{}/.config/PasswordManager/hash.txt", home_dir)).unwrap();
